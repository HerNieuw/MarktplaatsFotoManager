...
echo "📦 Stap 1b: Controleren en installeren van GTK/PyGObject (voor de GUI)..."
if ! python3 -c "import gi; gi.require_version('Gtk', '3.0')" &> /dev/null; then
    echo "   python3-gi/GTK3 niet gevonden. Installeren..."
    if command -v apt &> /dev/null; then
        sudo apt update && sudo apt install -y python3-gi python3-gi-cairo gir1.2-gtk-3.0
    elif command -v dnf &> /dev/null; then
        sudo dnf install -y python3-gobject gtk3
    elif command -v yum &> /dev/null; then
        sudo yum install -y python3-gobject gtk3
    else
        echo "⚠️  Kan GTK/PyGObject niet installeren. Installeer het handmatig via je package manager."
    fi
else
    echo "   ✅ GTK/PyGObject is aanwezig."
fi
