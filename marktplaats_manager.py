...
class ImagePreviewWindow(Gtk.Window):
    ...
    def save_rotation(self):
        ...
        try:
            ...
            # BELANGRIJK: de rotatie is nu permanent in het bestand gebakken,
            # dus rotation_angle terug naar 0. Zonder dit werd bij de volgende
            # update_display() dezelfde hoek NOG EEN KEER bovenop de al-
            # geroteerde pixbuf gezet - vandaar dat een enkele +90°-klik
            # zichtbaar naar 180° "sprong".
            self.rotation_angle = 0
            self.rotation_label.set_text("0°")
            self.rotation_scale.set_value(0)

            self.update_display()
            self.status_label.set_text(f"Rotatie opgeslagen")
            return True
        except Exception as e:
    ...
    def rotate_left(self, widget):
        # PIL's Image.rotate() draait tegen de klok in bij een POSITIEVE hoek.
        # Het ↺-icoon (tegen de klok in) moet dus +90 sturen, niet -90 -
        # anders draait de knop met het CCW-icoon in werkelijkheid met de klok mee.
...
class CustomFileChooser(Gtk.Dialog):
    ...
    def __init__(self, parent, title="Selecteer foto's", select_multiple=True, start_folder=None):
        ...
        if start_folder and os.path.exists(start_folder):
            self.current_folder = start_folder
        else:
            self.current_folder = os.path.expanduser("~/Downloads")
...
class MarktplaatsApp(Gtk.Window):
    ...
    def setup_ui(self):
        ...
        self.notebook.append_page(settings_page, Gtk.Label(label="Instellingen"))

        self.processing_page = self.create_processing_page()
        self.notebook.append_page(self.processing_page, Gtk.Label(label="Verwerking"))

        log_page = self.create_log_page()
        self.notebook.append_page(log_page, Gtk.Label(label="Logboek"))

        self.statusbar = Gtk.Statusbar()
        main_vbox.pack_start(self.statusbar, False, False, 0)
        self.status_context = self.statusbar.get_context_id("status")

        # Global key-press event voor Enter op verwerking pagina
        self.connect("key-press-event", self.on_window_key_press)

    def on_window_key_press(self, widget, event):
        """Global key-press handler voor Enter op verwerking pagina"""
        keyval = event.keyval

        if keyval == Gdk.KEY_Return or keyval == Gdk.KEY_KP_Enter:
            # Check of we op de verwerking pagina zijn (tab index 1)
            current_page = self.notebook.get_current_page()
            if current_page == 1:  # Verwerking tab
                if hasattr(self, 'next_project_btn') and self.next_project_btn.get_sensitive():
                    self.next_project(None)
                    return True
                elif hasattr(self, 'start_btn') and self.start_btn.get_sensitive():
    ...
    def create_processing_page(self):
        ...
        self.info_label.set_markup("<span size='large'>Klik op 'Start Verwerking' om te beginnen</span>")
    ...
    def browse_files_with_icons(self, widget):
        chooser = CustomFileChooser(self, "Selecteer foto's (grote pictogrammen)", select_multiple=True, start_folder=os.path.expanduser("~/Downloads"))
    ...
    def browse_source(self, widget):
        ...
        current_dir = self.config.get('source_dir', '')
    ...
    def browse_watermark(self, widget):
        ...
        current_dir = self.config.get('watermark_path', '')
        if not current_dir or not os.path.exists(current_dir):
            current_dir = os.path.expanduser("~/Downloads")
        dialog.set_current_folder(current_dir)
    ...
    def browse_logo(self, widget):
        ...
        current_dir = self.config.get('logo_path', '')
        if not current_dir or not os.path.exists(current_dir):
            current_dir = os.path.expanduser("~/Downloads")
        dialog.set_current_folder(current_dir)
    ...
    def browse_background_image(self, widget):
        ...
        current_dir = self.config.get('background_image', '')
        if not current_dir or not os.path.exists(current_dir):
            current_dir = os.path.expanduser("~/Downloads")
        dialog.set_current_folder(current_dir)
    ...
    def process_images(self):
        try:
            ...
            # ===== ALS ORIGINELE ACHTERGROND BEHOUDEN =====
            if is_original:
                self.log_message("🔵 ORIGINELE ACHTERGROND BEHOUDEN")
                self.log_message("   - Alleen auto-rotatie (indien aangevinkt)")
                self.log_message("   - Alleen verkleinen naar 50%")
                self.log_message("   - Geen vierkant maken")
                self.log_message("   - Geen achtergrond verwijdering/toevoeging")

                original_output_dir = os.path.join(self.config['output_dir'], "origineel_verkleind")
                self.original_temp_dir = original_output_dir
                ...
                for i, img in enumerate(source_files):
                    ...
                    output_path = os.path.join(original_output_dir, base_name)
            ...
            # ===== DE REST ALLEEN VOOR NIET-ORIGINEEL =====
    ...
    def process_after_inspection(self):
        ...
        # ===== ORIGINELE ACHTERGROND BEHOUDEN =====
        if is_original:
            ...
            for i, img in enumerate(self.transparent_pngs):
                if self.stop_flag:
                    self.log_message("Verwerking gestopt")
                    GLib.idle_add(self.reset_ui_after_stop)
                    return

                # Expliciet naar echte JPEG converteren i.p.v. een byte-kopie
                # met een misleidende .jpg-extensie. Als de bron transparant
                # is (bv. een PNG die al eerder van achtergrond was ontdaan),
                # wordt die netjes op een witte achtergrond gezet - anders
                # bleef de oorspronkelijke onderliggende foto na conversie
                # zichtbaar, omdat achtergrond-verwijdering alleen het alpha-
                # kanaal aanpast en niet de pixelkleuren zelf wist.
                jpg_path = os.path.join(temp_with_logo_dir, f"logo_{i:03d}.jpg")
                pil_img = Image.open(img)
                if pil_img.mode in ("RGBA", "LA") or (pil_img.mode == "P" and "transparency" in pil_img.info):
                    pil_img = pil_img.convert("RGBA")
                    achtergrond = Image.new("RGB", pil_img.size, (255, 255, 255))
                    achtergrond.paste(pil_img, mask=pil_img.split()[3])
                    achtergrond.save(jpg_path, "JPEG", quality=95)
                else:
                    pil_img.convert("RGB").save(jpg_path, "JPEG", quality=95)

                # Voeg logo/watermerk toe op de originele afbeelding
                if has_watermark:
                ...
                self.update_progress_safe(0.8 + (i / len(self.transparent_pngs)) * 0.1)
    ...
    def ask_for_article_number_original(self, files_with_logo, temp_with_logo_dir, temp_square_dir):
        ...
        # Zorg dat de OK knop de standaard is (reageert op Enter)
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if article_number:
                ...
                # Verwijder de origineel_verkleind map
                if self.original_temp_dir and os.path.exists(self.original_temp_dir):
                    shutil.rmtree(self.original_temp_dir, ignore_errors=True)
                    self.original_temp_dir = None
    ...
    def next_project(self, widget):
        ...
        # Ga automatisch terug naar het instellingen tab
        if self.notebook:
            self.notebook.set_current_page(0)  # Index 0 = Instellingen
        ...
        # Verwijder de origineel_verkleind map als die nog bestaat
        if self.original_temp_dir and os.path.exists(self.original_temp_dir):
            shutil.rmtree(self.original_temp_dir, ignore_errors=True)
            self.original_temp_dir = None
